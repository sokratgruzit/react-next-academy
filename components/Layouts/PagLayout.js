...
function PagLayout({ children, setSettings, setCommonData }) {
  const { data } = useFetch(
    `${process.env.NEXT_PUBLIC_DATA_URL}/api/data/settings`
  );
...
export default connect(null, mapDispatchToProps)(PagLayout);
