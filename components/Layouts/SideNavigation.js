  ...
  useEffect(() => {
    function handleResize() {
      ...
      if (width < 1023) {
        ...
        setContent("Wallet Connect");
      }
    }

    handleResize(); // Set initial content on mount
      ...
      <div className={styles.top}>
        <Link href="/">
          <div className={styles.logo}>
            <div>
              <img src="/svg/SideNavLogo.png" alt="logo" />
            </div>
          </div>
        </Link>
      ...
      <ul className={styles.list}>
        {links?.map((item) => (
          <li className={styles.listItem} key={item._id}>
            <Link className={styles.itemInner} href={item.url}>
