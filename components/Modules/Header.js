...
function Header({ commonData }) {
  ...
  const getHeaderLinks = async () => {
    const res = await fetchData(`${process.env.NEXT_PUBLIC_DATA_URL}/api/data/header`);
        ...
        <div className={styles.headerInner}>
          <div className={styles.right}>
            ...
            <nav className={styles.navigation}>
              <div
                ref={topicsWrap}
                className={`topics ${topicsBox ? "active" : ""} ${styles.customBtn}`}
              >
                <button
                  className={`${styles.item} ${topicsBox ? styles.active__item : ""}`}
                  onClick={() => setTopicsBox(!topicsBox)}
                >
                  Topics
                </button>
                <svg
                  className={`${styles.arrow} ${topicsBox ? styles.rotated : ""}`}
                  width='11'
                  height='6'
                  viewBox='0 0 11 6'
                  fill='none'
              ...
              <nav className={styles.navigation}>
                {headerLinks
                  ? headerLinks.map((item) => {
                    return (
                      <Link key={item._id} href={item.url} className={styles.item}>
                        {item.title}
                      </Link>
                    );
                  })
            ...
            >
              <div className={theme ? 'lightLogo' : 'darkLogo'}>
                ...
                >
                  {" "}
                  <line x1='2' y1='8' x2='30' y2='8' stroke={theme ? '#00050F' : 'white'}></line>{" "}
                  <line x1='2' y1='16' x2='30' y2='16' stroke={theme ? '#00050F' : 'white'}></line>{" "}
                  <line x1='2' y1='24' x2='30' y2='24' stroke={theme ? '#00050F' : 'white'}></line>{" "}
                </svg>
              </div>
            </button>
            <AnimatePresence>
